...
static void RegisterRandoStatUpgradeHooks() {
    ...
    COND_VB_SHOULD(VB_MAGIC_FILL_TARGET, IS_RANDO && RAND_GET_OPTION(RSK_MAGIC_STAT_UPGRADE), {
        ...
        if (level == 0 && gSaveContext.ship.quest.data.randomizer.comboNativeMagicLevel == 0)
            return;
        ...
        s16 cap = MagicStatCapacity(level, required, adjustable,
    ...
    COND_VB_SHOULD(VB_MAGIC_STEP_CAPACITY_TARGET, IS_RANDO && RAND_GET_OPTION(RSK_MAGIC_STAT_UPGRADE), {
        ...
        *target = cap;
        // Loading or swapping games rebuilds the meter from saved remaining
        // magic. Capacity animation must not grant a new refill.
        gSaveContext.magicFillTarget = std::min(gSaveContext.magicFillTarget, cap);
        *should = false;
    });
}

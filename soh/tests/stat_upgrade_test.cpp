...
// The stat fixture tests native damage values; the independent Din sword
// overlay has its own focused reaction tests.
extern "C" uint8_t DinFireSword_DamageEntry(PlayState*, Actor*, const ColliderInfo*, uint32_t, uint8_t vanillaEntry) {
    return vanillaEntry;
}
...
int main() {
    ...
    CHECK(RG_SEASON_WINTER == 554);
    CHECK(RSK_CROSSOVER_MARIO_MASK == 306);
    ...
    CHECK(fill == 50);
    // File load and a game handoff rebuild meter capacity while restoring the
    // saved remaining magic. Repeated capacity steps must not earn a refill.
    for (s16 remaining : { 0, 7, 50 }) {
        gSaveContext.magicFillTarget = remaining;
        for (int frame = 0; frame < 8; ++frame) {
            s16 capacity = 0;
            CHECK(!Dispatch(VB_MAGIC_STEP_CAPACITY_TARGET, &capacity));
            CHECK(capacity == 50);
            CHECK(gSaveContext.magicFillTarget == remaining);
        }
    }

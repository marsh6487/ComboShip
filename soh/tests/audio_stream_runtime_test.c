...
int main(void) {
    ...
    const size_t permanentCapacity = ARRAY_COUNT(gAudioContext.permanentCache);
    while (gAudioContext.permanentPool.count < permanentCapacity) {
        // Do not seed fake font entries: ComboShip has a larger permanent cache,
        // so those IDs would overlap the real font-load cases below.
        REQUIRE(AudioHeap_AllocPermanent(SAMPLE_TABLE, gAudioContext.permanentPool.count + 600, 16) != NULL);
    }
    REQUIRE(AudioHeap_AllocPermanent(FONT_TABLE, 999, 16) == NULL);
    REQUIRE(gAudioContext.permanentPool.count == permanentCapacity);

...
static s32 DinFireShield_OptionEnabled(void) {
    return CVarGetInteger(CVAR_ENHANCEMENT("DinFireShield"), 0) && ResourceMgr_IsAltAssetsEnabled();
}
...
static s32 DinFireShield_ItemEnabled(u16 itemId) {
    return itemId == ITEM_SHIELD_HERO && DinFireShield_OptionEnabled() &&
           ResourceMgr_FileExists(AdultLink_IsActive() ? sAdultBracer : sChildBracer);
}
...
static s32 DinFireShield_Eligible(PlayState* play, Player* player) {
    if (play == NULL || player == NULL || player != GET_PLAYER(play) || !DinFireShield_OptionEnabled() ||
        ...
        player->actor.scale.y <= 0.0f || (player->stateFlags1 & (PLAYER_STATE1_DEAD | PLAYER_STATE1_8000000)) ||
        (player->stateFlags2 & PLAYER_STATE2_20000000) || player->csAction != 0 ||
        play->transitionTrigger != TRANS_TRIGGER_OFF) {
        return false;
    }
    if (player->currentShield != PLAYER_SHIELD_HEROS_SHIELD || ExtEquip_GetShieldDLOverride() != NULL ||
        BossRemains_IsOdolwaWorn() || BossRemains_IsGohtWorn()) {
        return false;
    }
    return ResourceMgr_FileExists((AdultLink_IsActive()) ? sAdultBracer : sChildBracer) &&
...
static s32 DinFireShield_SameContext(PlayState* play, Player* player) {
    return sFire.play == play && sFire.player == player && sFire.scene == play->sceneId &&
           sFire.age == (AdultLink_IsActive() ? 0 : 1);
}

void* DinFireShield_HandDL(PlayState* play, Player* player, void* closedHand) {
    if (closedHand == NULL || !DinFireShield_Eligible(play, player) ||
        player->rightHandType != PLAYER_MODELTYPE_RH_SHIELD)
        return NULL;
    FireResources resources;
    Gfx* bracer = ResourceMgr_LoadGfxByName(AdultLink_IsActive() ? sAdultBracer : sChildBracer);
    if (bracer == NULL || !DinFireShield_Load(&resources))
        return NULL;
    Gfx* dl = GRAPH_ALLOC(play->state.gfxCtx, 5 * sizeof(Gfx));
    Gfx* p = dl;
    gSPDisplayList(p++, bracer);
...
void DinFireShield_Update(PlayState* play, Player* player) {
    ...
    if (!DinFireShield_SameContext(play, player) ||
        ...
        sFire.play = play;
        sFire.player = player;
        sFire.scene = play->sceneId;
        sFire.age = (AdultLink_IsActive() ? 0 : 1);
    }
    if (play->pauseCtx.state != 0 || play->pauseCtx.debugEditor != 0 || sFire.lastFrame == play->gameplayFrames) {
        return;
    }
    sFire.lastFrame = play->gameplayFrames;
    sFire.phase = (sFire.phase + 1) & 1023;
    const s32 guarding =
        (player->stateFlags1 & PLAYER_STATE1_400000) && player->rightHandType == PLAYER_MODELTYPE_RH_SHIELD;
    ...
    if (guarding && CVarGetInteger(CVAR_ENHANCEMENT("DinFireShieldSfx"), 0)) {
        ...
        if (DinFireShield_Load(&resources)) {
            ...
            Actor_PlaySfx(&player->actor, NA_SE_PL_ARROW_CHARGE_FIRE - SFX_FLAG);
        }
    }
}
...
static void DinFireShield_DrawFlames(PlayState* play, const FireResources* resources, u16 phase, f32 opacity) {
    ...
    const Color_RGBA8 core = CosmeticEditor_GetChangedColor(255, 225, 122, 255, "Custom.DinFireShieldCore");
    const Color_RGBA8 outer = CosmeticEditor_GetChangedColor(255, 43, 3, 255, "Custom.DinFireShieldOuter");

    OPEN_DISPS(play->state.gfxCtx);
    Mtx* mtx = Matrix_Finalize(play->state.gfxCtx);
...
int DinFireShield_DrawItem(PlayState* play, s16 drawId) {
    if (play == NULL || drawId != GID_SHIELD_HERO)
        return false;
    const u16 itemId = ITEM_SHIELD_HERO;

// Execute all six MM SW97 draw/lifecycle paths with real types and GBI.
...
static int geometryAssets, geometryLoadFailure, textureLoadFailure;
...
int32_t CVarGetInteger(const char* name, int32_t fallback) {
    if (!strcmp(name, "gCosmetic.Arrows.ElementalImpactSounds")) {
        REQUIRE(fallback == 0);
...
Color_RGBA8 CVarGetColor(const char* name, Color_RGBA8 fallback) {
    char expected[128];
    for (int i = 0; i < 2; ++i) {
        snprintf(expected, sizeof(expected), "%s.Color", current->colorKeys[i]);
        if (!strcmp(name, expected)) {
            return (Color_RGBA8){ colorValues[i].r, colorValues[i].g, colorValues[i].b, fallback.a };
        }
    }
...
uint8_t ResourceMgr_FileAltExists(const char* path) {
    REQUIRE(alt);
    if (!strcmp(path, current->material) || !strcmp(path, current->model))
        return 0;
...
uint8_t ResourceMgr_FileExists(const char* path) {
    if (!strcmp(path, current->material))
        return (geometryAssets & 1) != 0;
    if (!strcmp(path, current->model))
        return (geometryAssets & 2) != 0;
    REQUIRE(!"unexpected companion display list name");
    return 0;
}
void* ResourceGetDataByName(const char* path) {
    if (!strcmp(path, current->material))
        return (geometryLoadFailure & 1) ? NULL : (void*)path;
    if (!strcmp(path, current->model))
        return (geometryLoadFailure & 2) ? NULL : (void*)path;
    for (int i = 0; i < 2; ++i) {
        if (current->textures[i] != NULL && !strcmp(path, current->textures[i])) {
            return (textureLoadFailure & (1 << i)) ? NULL : (void*)path;
        }
    }
    REQUIRE(!"unexpected texture name");
    return NULL;
}
...
void Matrix_RotateXF(f32 angle, MatrixMode mode) {
}
void Matrix_RotateYF(f32 angle, MatrixMode mode) {
}
void Matrix_RotateZF(f32 angle, MatrixMode mode) {
}
...
void Actor_PlaySfx_Flagged2(Actor* actor, u16 id) {
...
static void setup(int impact) {
    ...
    geometryAssets = 3;
    geometryLoadFailure = textureLoadFailure = 0;
...
static void allMedallionLifecycles(void) {
    static const struct {
        ...
        { ArrowDark_Init, ArrowDark_Update, &shadow.alpha, &shadow.timer, ARROW_TYPE_SW97_DARK, ARROW_TYPE_SEED_DARK,
          200 },
        { ArrowIce_Init, ArrowIce_Update, &water.alpha, &water.timer, ARROW_TYPE_SW97_ICE, ARROW_TYPE_SEED_ICE, 100 },
        { ArrowWind_Init, ArrowWind_Update, &forest.alpha, &forest.timer, ARROW_TYPE_SW97_WIND, ARROW_TYPE_SEED_WIND,
          120 },
        { ArrowSoul_Init, ArrowSoul_Update, &spirit.alpha, &spirit.timer, ARROW_TYPE_SW97_SOUL, ARROW_TYPE_SEED_SOUL,
          130 },
        { ArrowFire_Init, ArrowFire_Update, &fire.alpha, &fire.timer, ARROW_TYPE_SW97_FIRE, ARROW_TYPE_SEED_FIRE, 160 },
        { ArrowLight_Init, ArrowLight_Update, &light.alpha, &light.timer, ARROW_TYPE_SW97_LIGHT, ARROW_TYPE_SEED_LIGHT,
          130 },
    };
    Player holder = { 0 };
    ...
    for (size_t variant = 0; variant < ARRAY_COUNT(cases); ++variant) {
        ...
        for (int seed = 0; seed < 2; ++seed) {
            for (int textureMode = 0; textureMode < 3; ++textureMode) {
                ...
                arrow.unk_260 = 50;
                play.actorCtx.actorLists[ACTORCAT_PLAYER].first = &holder.actor;
...
static void missingCompanionFallsBack(void) {
    static const char* nativeTypes[] = { "Fire", "Ice", "Ice", "Light", "Fire", "Light" };
    char material[128], model[128];
    for (size_t variant = 0; variant < ARRAY_COUNT(cases); ++variant) {
        current = &cases[variant];
        snprintf(material, sizeof(material), "__OTR__overlays/ovl_Arrow_%s/g%sArrowMaterialDL", nativeTypes[variant],
                 nativeTypes[variant]);
        snprintf(model, sizeof(model), "__OTR__overlays/ovl_Arrow_%s/g%sArrowModelDL", nativeTypes[variant],
                 nativeTypes[variant]);
        for (int impact = 0; impact < 2; ++impact) {
            for (int missing = 0; missing < 3; ++missing) {
                setup(impact);
                alt = 0;
                geometryAssets = missing;
                size_t count = draw();
                size_t materialIndex = displayList(count, material);
                REQUIRE(displayList(count, model) == materialIndex + 2);
            }
            for (int failed = 1; failed < 4; ++failed) {
                setup(impact);
                alt = 1;
                assets = 0;
                geometryLoadFailure = failed;
                size_t count = draw();
                size_t materialIndex = displayList(count, material);
                REQUIRE(displayList(count, model) == materialIndex + 2);
            }
...
static void failedTextureKeepsMaterial(void) {
    for (size_t variant = 0; variant < ARRAY_COUNT(cases); ++variant) {
        current = &cases[variant];
...
static void waterSoundLifecycle(void) {
    static const s16 projectileTypes[] = { ARROW_TYPE_SW97_ICE, ARROW_TYPE_SEED_ICE };
    Actor holder = { 0 };
    current = &cases[1];
...
int main(void) {
    ...
    missingCompanionFallsBack();
    failedTextureKeepsMaterial();
    ...
    puts("PASS: all six MM medallion draws, live Alt toggles, complete/absent/partial pairs, named loads, "
         ...
         "optional companion geometry and failed-texture fallback, Water sound lifecycle, default-off Fire/Light "
         "impact sounds");
    return 0;
}

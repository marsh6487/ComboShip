// The MM item and native lightning/pool/matrix code are real. Game input,
// animation, audio, camera calls, and unrelated particle allocation are fixtures.
// This verifies scheduling/render commands, not in-game appearance or the existing
// compatibility stubs behind the camera/dust paths.
#include "global.h"
#include "mods/nei_oot_compat.h"
#include "soh/_nei_compat_core.h"
// The production player unity TU receives these aliases from sm64_mario.c.
...
static EffectSs particles[100]; // MM Play_Init allocates 100 native particle slots.
...
static EffectSsProfile lightningInit = { EFFECT_SS_LIGHTNING, recordLightning };
...
int func_8002829C() {
    // This MM compatibility function is currently a no-op in production.
    ++dustCount;
    return 0;
}
...
s32 Camera_ChangeSetting(Camera* unused, s16 setting) {
    REQUIRE(setting == CAM_SET_TURN_AROUND);
    ++cameraRequests;
    return 0;
}
void Camera_SetCameraData(Camera* unused, s16 flags, void* a, void* b, s16 c, s16 d) {
    REQUIRE(flags == 4 && c == 10);
}
int func_8005B1A4(Camera* unused) {
    ++cameraRestores;
    return 0;
}
...
s32 CollisionCheck_SetAT(PlayState* context, CollisionCheckContext* collision, Collider* collider) {
    ...
    REQUIRE(ddCollider.elem.atDmgInfo.damage == 40 && ddCollider.elem.atDmgInfo.effect == 1);
    REQUIRE(ddCollider.elem.atDmgInfo.dmgFlags == 0x510); // MM Goron punch/pound and Zora boomerang.
    REQUIRE(ddCollider.dim.pos.x == 100 && ddCollider.dim.pos.y == 200 && ddCollider.dim.pos.z == -300);
    ++collisionCalls;
    return 0;
}
void Gfx_SetupDL61_Xlu(GraphicsContext* context) {
...
void* Lib_SegmentedToVirtual(void* pointer) {
    return pointer;
}
...
static void reset(void) {
    ...
    gfx.polyOpa.d = (Gfx*)(matrices + ARRAY_COUNT(matrices));
    ...
    gEffectSsOverlayTable[EFFECT_SS_LIGHTNING].profile = &lightningInit;
    rootCount = bombCount = smallBombCount = dustCount = magicSpent = animationChanges = animationUpdates = 0;
    collisionCalls = cameraRequests = cameraRestores = damaged = blocked = missingAnimation = 0;
    availableMagic = 96;
    forcedBranches = 1;
    randomState = 23;
}
...
static void testCastTimeline(void) {
    ...
    printf("PASS MM Demise timeline: %d roots, peak %d native lightning slots, 13 final explosions, 5 damage ticks\n",
...
int main(void) {
    ...
    puts("PASS MM Demise native draw, palette, scheduling, stop, allocation fallback, and gameplay invariants");
    return 0;
}

...
#else
    MM::SpinEffectTextureFactoryV0 factory;
    MM::SpinEffectTextureFactoryV1 factoryV1;
#endif
    if (f.version == 1)
...
// MM XML (N64_US and GC_US) names its native effect assets differently.
// Exercise both native namespaces: applying only the OoT profiles leaves MM's
// charge, released spin and elemental arrow replacements in the broken domain.
static void NativeMmTests() {
    constexpr uint32_t flags = TEX_FLAG_LOAD_AS_RAW | TEX_FLAG_LOAD_AS_IMG;
    const Profile mmProfiles[] = {
        { "objects/gameplay_keep/gSpinAttack1Tex", 64, 32, 6 },
        { "objects/gameplay_keep/gSpinAttack2Tex", 64, 32, 6 },
        { "objects/gameplay_keep/gSpinAttackCharging1Tex", 32, 32, 6 },
        { "objects/gameplay_keep/gSpinAttackCharging2Tex", 32, 32, 6 },
        { "objects/gameplay_keep/gEffFireTex", 32, 64, 6 },
        { "objects/gameplay_keep/gEffFireMaskTex", 32, 128, 5 },
        { "overlays/ovl_Arrow_Fire/gFireArrowTex", 32, 64, 6 },
        { "overlays/ovl_Arrow_Fire/gFireArrowMaskTex", 32, 64, 6 },
        { "overlays/ovl_Arrow_Ice/gIceArrowTex", 32, 64, 6 },
        { "overlays/ovl_Arrow_Ice/gIceArrowMaskTex", 32, 64, 6 },
        { "overlays/ovl_Arrow_Light/gLightArrowTex", 32, 64, 6 },
        { "overlays/ovl_Arrow_Light/gLightArrowMaskTex", 32, 64, 6 },
    };
    for (size_t i = 0; i < std::size(mmProfiles); ++i) {
        const auto& p = mmProfiles[i];
        const float byteScale = p.type == 5 ? 8 : 4;
        for (const auto& path : { std::string(p.path), "alt/" + std::string(p.path) }) {
            Promoted(path, p.width, p.height, 2, p.type);
            CheckV1(MakeV1(path, p.width * 2, p.height * 2, byteScale * 2, 2, TEX_FLAG_LOAD_AS_RAW, p.type), flags, 1,
                    1);
            Unchanged(path, p.width, p.height, p.type);
            Unchanged(path, p.width * 2, p.height * 2, p.type == 5 ? 6 : 5);
            Unchanged(path, p.width * 2, p.height * 3, p.type);
            Unchanged(path + "Suffix", p.width * 2, p.height * 2, p.type);
            CheckV1(MakeV1(path, p.width * 2, p.height * 2, 3, 7, flags, p.type), flags, 3, 7);
            CheckV1(MakeV1(path, p.width * 2, p.height * 2, byteScale, 2, TEX_FLAG_LOAD_AS_RAW, p.type),
                    TEX_FLAG_LOAD_AS_RAW, byteScale, 2);
            // Match the donor's four spin restrictions; ordinary effect profiles
            // also accept conventional RGBA32 and uniform fractional V1 images.
            CheckV1(MakeV1(path, p.width * 2, p.height * 2, byteScale * 2, 2, TEX_FLAG_LOAD_AS_RAW, 1),
                    i < 4 ? TEX_FLAG_LOAD_AS_RAW : flags, i < 4 ? byteScale * 2 : 1, i < 4 ? 2 : 1);
            CheckV1(
                MakeV1(path, p.width * 3 / 2, p.height * 3 / 2, byteScale * 1.5f, 1.5f, TEX_FLAG_LOAD_AS_RAW, p.type),
                i < 4 ? TEX_FLAG_LOAD_AS_RAW : flags, i < 4 ? byteScale * 1.5f : 1, i < 4 ? 1.5f : 1);
#ifndef SPIN_BASELINE_FACTORY
            auto malformed = Make(path, p.width * 2, p.height * 2, p.type);
            malformed.file->Buffer->pop_back();
            CHECK(Load(malformed) == nullptr);
            auto malformedV1 = MakeV1(path, p.width * 2, p.height * 2, byteScale * 2, 2, TEX_FLAG_LOAD_AS_RAW, p.type);
            malformedV1.file->Buffer->pop_back();
            CHECK(Load(malformedV1) == nullptr);
#endif
        }
    }
    // Similar-looking MM assets outside the verified scope must retain native parsing.
    Unchanged("objects/gameplay_keep/gSwordBeam1Tex", 64, 128);
    Unchanged("objects/gameplay_keep/gEffEnemyDeathFlame1Tex", 64, 128);
    std::puts("PASS 12 MM native profiles, both Alt/base namespaces, spin restrictions and bounds");
}
...
static void ActualResource(const char* resourcePath, const char* localPath, bool normalize) {
    ...
    // Compare untouched resources with the actual native parser, including its
    // authored scales and buffer ownership (e.g. the POC2 snowflake billboard).
    Fast::ResourceFactoryBinaryTextureV0 nativeV0;
    Fast::ResourceFactoryBinaryTextureV1 nativeV1;
    auto native = std::static_pointer_cast<Fast::Texture>(version == 0 ? nativeV0.ReadResource(f.file, f.init)
                                                                       : nativeV1.ReadResource(f.file, f.init));
    std::get<std::shared_ptr<Ship::BinaryReader>>(f.file->Reader)->Seek(64, Ship::SeekOffsetType::Start);
    auto t = Load(f);
    CHECK(t);
    if (normalize) {
        CHECK(t->Flags == (TEX_FLAG_LOAD_AS_RAW | TEX_FLAG_LOAD_AS_IMG));
        ...
        CHECK(t->Flags == native->Flags && t->HByteScale == native->HByteScale &&
              t->VPixelScale == native->VPixelScale);
        CHECK(t->Type == native->Type && t->Width == native->Width && t->Height == native->Height);
        CHECK(t->ImageData == native->ImageData && t->ImageDataSize == native->ImageDataSize);
    }
    if (version == 1 || !normalize) {
...
int main(int argc, char** argv) {
    if (argc >= 4 && (argc - 1) % 3 == 0 &&
        (!std::strcmp(argv[1], "--resource") || !std::strcmp(argv[1], "--preserve-resource"))) {
        for (int i = 1; i < argc; i += 3) {
            CHECK(!std::strcmp(argv[i], "--resource") || !std::strcmp(argv[i], "--preserve-resource"));
            ActualResource(argv[i + 1], argv[i + 2], !std::strcmp(argv[i], "--resource"));
        }
        return 0;
    }
    const struct {
        ...
        { "mm", NativeMmTests }, { "legacy", LegacyTests },         { "profiles", ProfileTests },    { "i4", I4Tests },

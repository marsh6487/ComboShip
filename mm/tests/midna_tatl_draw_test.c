...
static Vtx pose[1];
static int enabled = 1, giantMask, cueId;
...
Gfx* testLoadGfx(const char* path) {
    ++resourceLoads;
    int blink = blinkResource(path);
    if (blink == 1 || blink == 2)
...
char* testLoadTexture(const char* path) {
    int blink = blinkResource(path);
    if (blink == 3 || blink == 4)
...
void Gfx_SetupDL27_Xlu(GraphicsContext* gfx) {
}
...
s16 Math_Atan2S_XY(f32 y, f32 x) {
    return 0;
}
...
void Matrix_Scale(f32 x, f32 y, f32 z, MatrixMode mode) {
     ...
     * native Tatl scale 0.008. Apply the same factor on every axis. */
...
s32 EnElf_OverrideLimbDraw(PlayState* play, s32 limb, Gfx** dl, Vec3f* p, Vec3s* r, Actor* actor, Gfx** gfx) {
    return 0;
}
...
void* MMMidnaResources_Load(const char* path) {
    if (strstr(path, "DL"))
        return testLoadGfx(path);
    if (strstr(path, "Pose") || strstr(path, "Vertices"))
        return testLoadVtx((char*)path);
    return testLoadTexture(path);
}
int32_t CVarGetInteger(const char* path, int32_t fallback) {
    REQUIRE(strcmp(path, "gEnhancements.MidnaCompanionMM") == 0 && fallback == 0);
    return enabled;
}
s32 Cutscene_IsCueInChannel(PlayState* play, u16 type) {
    return cueId != 0;
}
s32 Cutscene_GetCueChannel(PlayState* play, u16 type) {
    return 0;
}
void func_8088F9E4(Actor* fairy, PlayState* play) {
}
...
static void checkCase(int fairyType, int present, int loaded, int hiddenState, int hiddenFlag, int firstPerson,
    ...
    gfx.polyOpa.d = allocated + 4;
    play.state.gfxCtx = &gfx;
    play.actorCtx.actorLists[ACTORCAT_PLAYER].first = &player.actor;
    player.currentMask = giantMask ? PLAYER_MASK_GIANT : PLAYER_MASK_NONE;
    CsCmdActorCue cue = { 0 };
    cue.id = hiddenState ? 6 : 5;
    cueId = hiddenState ? 6 : 0;
    play.csCtx.actorCues[0] = &cue;
    player.stateFlags1 = firstPerson ? PLAYER_STATE1_100000 : 0;
    ...
    if (fairyType != FAIRY_TYPE_0 || !enabled || giantMask || hiddenState || hiddenFlag || (firstPerson && !inFront)) {
...
static void checkLights(int fairyType, int present, int loaded, int hidden, int expectType) {
    ...
    cueId = 0;
    fairy.actor.params = fairyType;
    fairy.unk_244 = hidden ? 6 : 0;
    packPresent = present;
    loadSucceeds = loaded;
    func_8088E60C(&fairy, &play);
...
static void checkVisibleClock(void) {
    ...
    expectedBlink = 0;
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    fairy.unk_244 = 0;
    for (int i = 1; i <= 230; ++i) {
        EnElf_Update(&fairy.actor, &play);
        inputTimer = fairy.timer;
    ...
    REQUIRE(fairy.midnaBlinkTimer == held);
    fairy.innerColor.a = 255;
    fairy.actor.params = FAIRY_TYPE_2;
    EnElf_Update(&fairy.actor, &play);
    REQUIRE(fairy.midnaBlinkTimer == held);
#endif
...
int main(void) {
    enabled = 0;
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 0, 1);
    enabled = 1;
    giantMask = 1;
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 0, 0);
    giantMask = 0;
    /* Installed pack selects Midna only for the companion fairy. */
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    for (int type = FAIRY_TYPE_1; type <= FAIRY_TYPE_10; ++type)
        checkCase(type, 1, 1, 0, 0, 0, 1, 0, 1);
    /* Removing the pack or a failed load retains the native draw. */
    checkCase(FAIRY_TYPE_0, 0, 1, 0, 0, 0, 1, 0, 1);
    checkCase(FAIRY_TYPE_0, 1, 0, 0, 0, 0, 1, 0, 1);
    checkCase(FAIRY_TYPE_0, 1, 1, 1, 0, 0, 1, 0, 0);
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 1, 0, 1, 0, 0);
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 1, 0, 0, 0);
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 1, 1, 1, 0);
    ...
    checkCase(FAIRY_TYPE_0, 0, 0, 0, 0, 0, 1, 0, 1);
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    inputAlpha = 128;
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    inputAlpha = 0;
    ...
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    textureFailed = 0;
    checkCase(FAIRY_TYPE_2, 1, 1, 0, 0, 0, 1, 0, 1);
    inputAlpha = 128;
    checkCase(FAIRY_TYPE_0, 1, 1, 0, 0, 0, 1, 1, 0);
    checkLights(FAIRY_TYPE_0, 1, 1, 0, LIGHT_POINT_NOGLOW);
    checkLights(FAIRY_TYPE_0, 1, 1, 1, LIGHT_POINT_NOGLOW);
    checkLights(FAIRY_TYPE_0, 0, 0, 0, LIGHT_POINT_GLOW);
    checkLights(FAIRY_TYPE_0, 1, 0, 0, LIGHT_POINT_GLOW);
    checkLights(FAIRY_TYPE_2, 1, 1, 0, LIGHT_POINT_GLOW);
    ...
    puts("PASS: Midna is Tatl-only; fallback, visibility, fade depth, actor state and matrix balance");
    return 0;
}

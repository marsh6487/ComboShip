...
void FireArrow_ChargeAndWait(ArrowFire*, PlayState*);
void FireArrow_Fly(ArrowFire*, PlayState*);
void FireArrow_Hit(ArrowFire*, PlayState*);
...
int32_t CVarGetInteger(const char* key, int32_t fallback) {
    REQUIRE(!strcmp(key, "gCosmetic.Arrows.ElementalImpactSounds"));
    REQUIRE(fallback == 0);
...
Color_RGBA8 CosmeticEditor_GetChangedColor(u8 r, u8 g, u8 b, u8 a, const char* key) {
    REQUIRE(!strcmp(key, COSMETIC_ID("Effects.IceArrowPrim")) || !strcmp(key, COSMETIC_ID("Effects.IceArrowSec")));
    if (!customColors) {
        return (Color_RGBA8){ r, g, b, a };
    }
    return strstr(key, "Prim") ? (Color_RGBA8){ 241, 73, 19, a } : (Color_RGBA8){ 11, 23, 47, a };
}
void gDPSetPrimColorOverride(Gfx* p, u8 m, u8 l, u8 r, u8 g, u8 b, u8 a, const char* key) {
    Color_RGBA8 color = CosmeticEditor_GetChangedColor(r, g, b, a, key);
    gDPSetPrimColor(p, m, l, color.r, color.g, color.b, a);
}
void gDPSetEnvColorOverride(Gfx* p, u8 r, u8 g, u8 b, u8 a, const char* key) {
    Color_RGBA8 color = CosmeticEditor_GetChangedColor(r, g, b, a, key);
    gDPSetEnvColor(p, color.r, color.g, color.b, a);
}
...
void Matrix_RotateXS(s16 a, MatrixMode mode) {
}
void Matrix_RotateYS(s16 a, MatrixMode mode) {
}
void Matrix_RotateZF(f32 a, MatrixMode mode) {
    lastRotation = mode == MTXMODE_NEW ? a : lastRotation + a;
}
void Matrix_RotateZS(s16 a, MatrixMode mode) {
}
...
static int nativeGeometry(size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        if ((commands[i].words.w0 >> 24) != G_DL || commands[i].words.w1 == (uintptr_t)scroll ||
            (commands[i].words.w1 & 1))
            continue;
        const char* path = (const char*)commands[i].words.w1;
        REQUIRE(path != NULL);
        if (!strcmp(path, "__OTR__overlays/ovl_Arrow_Ice/gIceArrowMaterialDL") ||
            !strcmp(path, "__OTR__overlays/ovl_Arrow_Ice/gIceArrowModelDL"))
            ++found;
    }
    return found;
}
static void hit(u16 timer) {
    ...
    ice.timer = timer;
    ice.alpha = 255;
    ice.blueingEffectMagnitude = 1.0f;
}
...
static void nativeIceLifecycle(void) {
    Actor holder = { 0 };
    setup();
    ice.actor.update = alive;
    arrow.actor.parent = &holder;
    arrow.unk_260 = 50;
    ArrowIce_Charge(&ice, &play);
    ...
    arrow.actor.parent = NULL;
    ArrowIce_Charge(&ice, &play);
    REQUIRE(ice.actionFunc == ArrowIce_Fly && ice.alpha == 255);
    audioCalls = 0;
    ArrowIce_Fly(&ice, &play);
    REQUIRE(audioCalls == 0);
    arrow.unk_261 = 1;
    ArrowIce_Fly(&ice, &play);
    REQUIRE(audioCalls == 1 && lastAudioActor == &ice.actor && !lastAudioFlagged);
    REQUIRE(lastAudioId == NA_SE_IT_EXPLOSION_ICE);
    REQUIRE(ice.actionFunc == ArrowIce_Hit && ice.timer == 32 && ice.alpha == 255);
    for (int frame = 0; frame < 32; ++frame) {
        ArrowIce_Hit(&ice, &play);
        REQUIRE(audioCalls == 1);
    }
    REQUIRE(ice.actor.update == NULL && ice.timer == 255 && ice.alpha == 0);
}
...
static void nativeImpactSounds(void) {
    ...
    for (size_t step = 0; step < ARRAY_COUNT(options); ++step) {
        ...
        arrow.actor.parent = &holder;
        arrow.unk_260 = 50;
        FireArrow_ChargeAndWait(&fire, &play);
        ...
        REQUIRE(fire.actionFunc == FireArrow_Hit && fire.timer == 32 && fire.alpha == 255);
        FireArrow_Hit(&fire, &play);
        REQUIRE(audioCalls == 1);
...
int main(void) {
    ...
    puts("PASS: continuous custom charge/release/flight/impact, gentle held motion, phase/layer interpolation, "
         ...
         "cosmetics, pause, draw-only state preservation, MM native Fire/Ice/Light default-off impact sounds and ice "
         "lifetime");
    return 0;
}

...
static inline Gfx* Sw97_SpellScrollWithAppearance(GraphicsContext* gfxCtx, Gfx* scroll, const char* first,
                                                  const char* second, s32 imageSize, const Color_RGBA8* primary,
                                                  const Color_RGBA8* secondary) {
    const u8 textures = ResourceMgr_IsAltAssetsEnabled() && ResourceMgr_FileAltExists(first) &&
                        ResourceMgr_FileAltExists(second) && ResourceGetDataByName(first) != NULL &&
                        ResourceGetDataByName(second) != NULL;
    if (!textures && primary == NULL && secondary == NULL) {
        return scroll;
    }

    Gfx* result = (Gfx*)GRAPH_ALLOC(gfxCtx, 15 * sizeof(Gfx));
    if (result == NULL) {
        return scroll;
    }
    Gfx* gfx = result;

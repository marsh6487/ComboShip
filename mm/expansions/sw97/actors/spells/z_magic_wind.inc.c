...
// Keep the native dust motion/lifetime; only this tornado gets the private appearance tag.
static void MagicWind_SpawnTornadoVFX(PlayState* play, Vec3f* c, f32 R) {
    ...
    for (s32 i = 0; i < 12; i++) {
        ...
        EffectSsDust_Spawn(play, EFFECT_SS_DUST_DRAW_SW97_FOREST, &pos, &vel, &accel, &prim, &env, 200, 30, 16,
                           DUST_UPDATE_NORMAL);
    }
    ...
    for (s32 i = 0; i < 8; i++) {
        ...
        EffectSsDust_Spawn(play, EFFECT_SS_DUST_DRAW_SW97_FOREST, &pos, &vel, &accel, &prim, &env, 200, 30, 18,
                           DUST_UPDATE_NORMAL);
    }
...
s32 MagicWind_OverrideLimbDraw(PlayState* play, SkelCurve* skelCurve, s32 limbIndex, void* thisx) {
    MagicWind* this = THIS;
    Color_RGBA8 primary = { 255, 255, 170, 255 };
    Color_RGBA8 secondary = { limbIndex == 1 ? 150 : 0, limbIndex == 1 ? 255 : 150, 0, 0 };

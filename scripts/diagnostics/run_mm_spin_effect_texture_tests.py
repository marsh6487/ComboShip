"""Run the real MM effect texture importers with production binary readers and ownership."""
...
def main():
    sources = [
        "mm/tests/spin_effect_texture_test.cpp",
        "mm/2s2h/resource/importer/SpinEffectTextureFactory.cpp",
    ...
    if baseline:
        ...
        sources.remove("mm/2s2h/resource/importer/SpinEffectTextureFactory.cpp")
    with tempfile.TemporaryDirectory(prefix="mm-spin-effect-textures-") as directory:
        temp = Path(directory)
        ...
        subprocess.run([
            ...
            *(["-DSPIN_BASELINE_FACTORY"] if baseline else []),
        ...
        subprocess.run([str(binary), *arguments], check=True, env=environment)

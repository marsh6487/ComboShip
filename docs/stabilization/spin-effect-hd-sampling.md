...
ComboShip integration: ported Shipwright commit
`51889826b9c0cea8b6bdbdeb45749de715e12ec0` onto cosmetics candidate
`904b8dbbec4bed8eddd17591471c414400316af5`. The existing OoT texture-factory
registrations now use the donor wrappers; MM's registrations are unchanged.
The CMake merge retains both the existing pak-selection test and the donor
sampling test, and places the Windows sampling executable beside the shared
engine DLLs. The imported test fails with ComboShip's original factories and
passes with the wrappers using AddressSanitizer and UndefinedBehaviorSanitizer;
only logging macros are stubbed in this local check. Full application builds
and in-game acceptance remain pending. The donor implementation and its
acceptance limits follow.
